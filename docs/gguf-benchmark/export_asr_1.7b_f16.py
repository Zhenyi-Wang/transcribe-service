"""导出 1.7B ASR Decoder 为 F16 GGUF"""
import sys
import os
import json
import torch
...
# 模型路径 - 使用简化路径绕过 HFValidationError
ASR_MODEL_DIR = "/tmp/qwen3-asr-1.7b"
EXPORT_DIR = os.path.expanduser("~/ownprojects/transcribe-service/model/asr_decoder_hf")
MODEL_GGUF = os.path.expanduser("~/ownprojects/transcribe-service/model/qwen3_asr_llm.f16.gguf")

os.makedirs(EXPORT_DIR, exist_ok=True)

# Step 1: 提取 Decoder 权重
print("=" * 60)
print("Step 1: 提取 ASR Decoder 权重")
print("=" * 60)

sys.path.insert(0, "/home/zhenyi/.local/lib/python3.10/site-packages")
from qwen_asr.core.transformers_backend.modeling_qwen3_asr import Qwen3ASRForConditionalGeneration

print(f"Loading model from: {ASR_MODEL_DIR}")
model = Qwen3ASRForConditionalGeneration.from_pretrained(
    ASR_MODEL_DIR, trust_remote_code=True, device_map="cpu", torch_dtype=torch.float32,
...
# Monkey patch: 强制使用本地 config.json
